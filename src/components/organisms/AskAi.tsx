      ...
      <div className="w-full p-4">
        <div className="bg-white rounded-lg p-6 flex flex-col relative w-full">
          <div className="flex items-center mb-6">
            <img src={coterieBot} alt="Coterie Bot" className="w-10 h-10 rounded-full mr-3" />
            <h2 className="font-black text-3xl text-primary">Coterie AI</h2>
            <img src={moreOptions} alt="More Options" className="ml-auto cursor-pointer" />
          </div>
              ...
              <div key={index} className="relative">
                {message.time && (
                  <div className="flex items-center my-4">
                    ...
                    <span className="mx-2 text-gray-500 text-sm">{message.time}</span>
                    <div className="flex-grow border-t border-gray-400"></div>
                  </div>
                )}
                <div className={`flex ${index % 2 === 0 ? 'justify-end' : 'justify-start'} items-end`}>
                  {index % 2 === 0 ? (
                    <>
                      <div className="ml-3 basis-4/5 text-right">
                        <p
                          className="p-3 rounded-lg"
                          style={index === 0 || index === 2 ? { background: '#F5F6FB' } : { background: 'bg-gray-100' }}
                        >
                          {message.text}
                        </p>
                      </div>
                      <img src={profilePicture} alt="Profile" className="w-10 h-10 rounded-full flex-shrink-0 mt-1" />
                    </>
                  ) : (
                    <>
                      <img src={coterieBot} alt="Coterie Bot" className="w-10 h-10 rounded-full flex-shrink-0 mt-1" />
                      <div className="ml-3 basis-4/5 text-left">
                        <p
                          className="p-3 rounded-lg"
                          style={index === 1 ? { background: 'var(--Main-50, #E9F3FF)' } : index === messages.length - 1 ? { background: 'var(--Main-800, #1026C3)', color: 'white' } : { background: 'bg-blue-100', color: 'text-blue-800' }}
                        >
                          {message.text}
                        </p>
                      </div>
          ...
          <div className="w-full mt-4">
            <div
              ...
              style={{ background: '#F5F6FB', width: '742px', height: '76px', gap: '10px' }}
            >
              <input
                type="text"
              ...
              />
              <button className="p-2 rounded-full text-white ml-2" style={{ background: '#F5F6FB' }}>
